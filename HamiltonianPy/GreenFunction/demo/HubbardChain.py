...
omegas = np.arange(-3, 3, step)
kpoints = np.dot(np.arange(-0.5, 0.5 + step, step).reshape((-1, 1)), cell.bs)

intra_bonds, inter_bonds = cluster.bonds(nth=1)
HTerms = [HP.HubbardFactory(site=site, coeff=U/2) for site in cluster.points]
for bond in intra_bonds:
    p0, p1 = bond.endpoints
    for spin in SPINS:
        HTerms.append(HP.HoppingFactory(p0, p1, spin0=spin, coeff=t))
...
Cell_Cs = [
    HP.AoC(otype=HP.CREATION, site=site, spin=spin)
...
for bond in inter_bonds:
    ...
    for spin in SPINS:
        C = HP.AoC(otype=HP.CREATION, site=p0_eqv, spin=spin)
        A = HP.AoC(otype=HP.ANNIHILATION, site=p1_eqv, spin=spin)
