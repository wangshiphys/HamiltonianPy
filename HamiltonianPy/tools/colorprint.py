...
def _color_name(color):
    ...
    color = color.lower()
    if color in COLOR_NAMES_STD_LONG:
        color_name = color
    elif color in COLOR_NAMES_STD_SHORT:
        color_name = SHORT2LONG[color]
    else:
...
# If `fg` set to True, these functions control the foreground color
# If `fg` set to False, these functions control the background color
...
def _color_ctrl_seq_4bits(color=None, fg=True):
    if isinstance(color, int):
        ...
        if color in supported:
            ...
            raise ValueError(
                "The given `color` is not supported by 4-bits color scheme."
            )
...
def _color_ctrl_seq_8bits(color=None, fg=True):
    if isinstance(color, int):
        if color in COLOR_CODES_ALL_8BITS:
            ...
            raise ValueError(
                "The given `color` is not supported by 8-bits color scheme."
            )
    elif isinstance(color, str):
...
def _color_ctrl_seq_24bits(color=None, fg=True):
    if isinstance(color, (list, tuple)) and len(color) == 3:
        r, g, b = color
        if not (
            r in COLOR_CODES_ALL_24BITS_R and
            g in COLOR_CODES_ALL_24BITS_G and
            b in COLOR_CODES_ALL_24BITS_B
        ):
            raise ValueError(
                "The given `color` is not supported by 24-bits color scheme."
            )
    elif isinstance(color, str):
...
def coloring(string, fg=None, bg=None, bold=False, color_scheme=8):
    """
    ...
    if color_scheme == 4:
        ...
        raise ValueError("Invalid `color_scheme` parameter.")
...
def color_print(
        *objects, sep=' ', end='\n', file=None, flush=False, fg=None,
        bg=None, bold=False, color_scheme=8
):
    """
    Print the given `objects` with color
    ...
    Note:
        ...
        It is not appropriate to print to a text file although it is allowed by
    ...
    sep : str, optional, keyword-only
        string inserted between objects
        default : a space
    ...
    tmp = sep.join(str(obj) for obj in objects)
    colored_tmp = coloring(
        tmp, fg=fg, bg=bg, bold=bold, color_scheme=color_scheme
    )
    print(colored_tmp, end=end, file=file, flush=flush)


def color_demo(with_bg=False, space_num=0):
    """
    Demonstrate all colors of the 8-bits color scheme
    ...
    prefix = ' ' * space_num
    print(prefix, end='')
    color_print("fg = None, bg = None", fg=None, bg=None)
    for fg in range(256):
        print(prefix, end='')
        color_print("fg = {0:>4d}, bg = None".format(fg), fg=fg, bg=None)

    if with_bg:
        for bg in range(256):
            print(prefix, end='')
            color_print("fg = None, bg = {0:>4d}".format(bg), fg=None, bg=bg)
        for fg in range(256):
            for bg in range(256):
                content = "fg = {0:>4d}, bg = {1:>4d}".format(fg, bg)
                print(prefix, end='')
                color_print(content, fg=fg, bg=bg)
...
if __name__ == "__main__":
    ...
    for index, fg in enumerate(product(range(256), repeat=3)):
        color_print(" " * 60 + str(index), fg=fg, bold=True, color_scheme=24)
